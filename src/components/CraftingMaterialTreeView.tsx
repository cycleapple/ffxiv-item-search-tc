// Material list view for a single crafting item - shows deduplicated materials
...
export function CraftingMaterialTreeView({ tree, qualityFilter, showCrystals, showOwned = false, ownedMaterials = {}, onOwnedChange }: CraftingMaterialTreeViewProps) {
  ...
  const { maxDepth, allMaterials, materialChildren, materialParents } = useMemo(() => {
    ...
    // Calculate max depth to check if there are any materials
    let max = 0;
    for (const mat of materials.values()) {
      max = Math.max(max, mat.depth);
    }

    return { maxDepth: max, allMaterials: matById, materialChildren: childrenMap, materialParents: parents };
  }, [tree, showCrystals]);
  ...
  // Sort materials: non-crystals first, craftable first, then by quantity descending
  const sortedMaterials = useMemo(() => {
    return Array.from(allMaterials.values()).sort((a, b) => {
      ...
      const aCraftable = a.hasRecipe ? 0 : 1;
      const bCraftable = b.hasRecipe ? 0 : 1;
